...
![按下Shift + P](../../assets/images/zotero-plugin-style/zoterostyle-shiftp菜单.png)
...
![按下Shift + P](../../assets/images/zotero-plugin-style/zoterostyle-阅读进度条shiftp.png)
...
* `Tags` ：勾选后，Zotero 的标签会在标题前显示；或者使用[标签列](#标签) *Z6*
* `Color` ：阅读进度条的颜色
* `Opacity`：透明度；如果你想关闭标题进度条，可以把它拉到开头，也就是 opacity=0
* `Odd Color` :奇数行条目背景色 *Z6*
* `Even Color` ：偶数行条目背景色 *Z6*
* `Selected Color` ：选中行条目背景色 *Z6*
* `Translate`：在标题上显示翻译 *Z7*
