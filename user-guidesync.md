...
目前坚果云官方团队已经开发了 Zotero 的坚果云官方 Zotero 同步插件 [Nutstore SSO](/plugins/#search=Nutstore%20SSO)，可以直接通过插件登录坚果云账户并自动配置好 WebDAV。坚果云官方同步插件的使用方法请阅读： [坚果云官方 Zotero 同步插件使用教程](https://mp.weixin.qq.com/s/w9pzcuikfJ7hC9t3yVa-qw)。
...
在使用这一方案时，附件会由「存储的附件」转换为「链接的附件」，这样会导致无法直接通过 Zotero 本身的「删除条目」功能来删除附件。您需要使用 Zotero 的「显示文件位置」功能，手动删除文件夹中的文件。此时，您可能需要借助 [Del Item With Attachment 插件](/plugins/#search=Del+Item+With+Attachment)提供的「删除条目和附件」功能进行删除。详见：[删除 Zotero 中链接的附件时，同时删除系统中的文件](./faqs/sync#教程-删除-zotero-中链接的附件时-同时删除系统中的文件)
